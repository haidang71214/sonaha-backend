...
export class PropritiesService {
  ...
  async create(id: number, createProprityDto: CreatePropertyDto): Promise<string> {
    ...
    async findAll(checkAdmin: number): Promise<GetAllPropertiesDto[]> {
      // chỗ đằng trên
      ...
      if(checkUser.role_name === 'admin' || checkUser.role_name === 'moderator' || checkUser.role_name === 'manager'){
        try {
          ...
          return plainToInstance(GetAllPropertiesDto,renderAllProperties);          
        } catch (error) {
...
// nếu như trong này cập nhật là sold thì bên khác cũng cập nhật bản ghi
  ...
  async findOne(id: number):Promise<GetAllPropertiesDto> {
    try {
      ...
      return plainToInstance(GetAllPropertiesDto,detail)
    } catch (error) {
      return error
    }
  }
...
async remove(userId: number, id: number): Promise<string> {
  try {
    ...
    if (checkAdmin.role_name === 'admin' || checkAdmin.role_name === 'manager') {
      ...
      // Tìm tất cả các comment liên quan
      ...
      // Xóa các notifications liên quan đến comment
    ...
    // Xóa các bản ghi trong bảng transactions
    ...
    // Xóa các bản ghi trong bảng type_properties
    ...
    // const transaction_date // nhớ xóa thống kê khi đạt 1 mức nhất định ha
