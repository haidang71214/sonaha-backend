...
prisma = new PrismaClient()
  ...
  async create(@Body() createProprityDto: CreatePropertyDto,
...
async findAll(
  @Req() req,
  @Res() res: Response
): Promise<GetAllPropertiesDto[]> { // Chỉnh sửa kiểu trả về
  try {
    const userId = req.user.userId;
    const listProperties = await this.propritiesService.findAll(+userId);
...
async update(
  ...
  try {
    ...
    await this.propritiesService.updateProperty(+id, +userId, updateProprityDto);
